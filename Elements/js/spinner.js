...
//To be able to tell where the bug is happening
...
function getNewAngle(event){
		...
		dXImgSpin=(centerXImgSpin-expansionOffsetXImgSpin)-(event.clientX);
		dYImgSpin=(centerYImgSpin-expansionOffsetYImgSpin)-(event.clientY);
...
function changeImgSpin(){
	...
	if (imgIsSpinning==0){
		imgSrcChangeSpin = "Elements/images/navBar/unSpin.png";
		imgDestChangeSpin.setAttribute("src", imgSrcChangeSpin);
		saveInstance();
		imgIsSpinning=1;
	}else{
		imgSrcChangeSpin = "Elements/images/navBar/spin.png";
		imgDestChangeSpin.setAttribute("src", imgSrcChangeSpin);
		killSpin();
		imgIsSpinning=0;
	}
